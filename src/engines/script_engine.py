...
以下のプロバイダーに対応:
- ollama   : ローカルOllama (localhost:11434) — デフォルト
- openai   : OpenAI互換API (LiteLLM / cocoro-llm-server等)
...
# デフォルトプロバイダー (Ollamaがローカルで稼働中のためデフォルトに設定)
...
- textは自然な日本語の話し言葉
- cinematic_promptはscene_typeが"cinematic"のときのみ英語で記述
...
class ScriptEngine(BaseEngine):
    ...
    def __init__(
        ...
        Args:
            ...
            base_url: APIのベースURL (ollama/openai用。Noneの場合はデフォルト)
        """
        super().__init__()
    ...
    def _default_model(self, provider: str) -> str:
        ...
        defaults = {
            "ollama": "qwen2.5:32b-instruct-q4_K_M",  # サーバーにインストール済み
            "openai": "gpt-4o-mini",                   # cocoro-llm-server エイリアス
            "gemini": "gemini-2.0-flash",
            "anthropic": "claude-3-5-haiku-latest",
        }
        return defaults.get(provider, "qwen2.5:32b-instruct-q4_K_M")

    def _default_base_url(self, provider: str) -> str | None:
        """プロバイダーごとのデフォルトbase_urlを返す"""
        defaults: dict[str, str] = {
            "ollama": os.environ.get("OLLAMA_BASE_URL", "http://localhost:11434"),
            "openai": os.environ.get("LLM_SERVER_URL", "http://192.168.50.112:8000"),
        }
    ...
    def load(self) -> None:
        ...
        logger.info(
            "ScriptEngine: ロード完了 (provider=%s, model=%s, base_url=%s)",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if self._provider == "ollama":
            raw_json = self._call_ollama(user_prompt)
        elif self._provider == "openai":
            raw_json = self._call_openai_compat(user_prompt)
    ...
    def _call_ollama(self, user_prompt: str) -> str:
        ...
        format="json" を指定してJSON出力を強制する。
        タイムアウトは300秒（32Bモデルは生成に時間がかかるため）。
        """
        ...
        try:
            ...
            if not response_text:
                raise RuntimeError("Ollamaからの応答が空です")
            logger.info("ScriptEngine: Ollama応答受信 (chars=%d)", len(response_text))
            return response_text
        except OSError as e:
            raise RuntimeError(
                f"Ollama接続エラー ({base_url}): {e}\n"
                "Ollamaサービスが起動しているか確認: systemctl status ollama"
            ) from e
        except Exception as e:
            raise RuntimeError(f"Ollama API呼び出しエラー: {e}") from e

    def _call_openai_compat(self, user_prompt: str) -> str:
        """OpenAI互換API を呼び出す (LiteLLM / cocoro-llm-server)

        cocoro-llm-server (192.168.50.112:8000) が起動していれば
        Llama 4 Scout / Qwen 3.5 32B に自動ルーティングされる。
        環境変数 LLM_SERVER_URL で接続先を変更可能。
        """
        import urllib.request

        base_url = self._base_url or "http://192.168.50.112:8000"
        url = f"{base_url}/v1/chat/completions"
        api_key = self._api_key or os.environ.get("LLM_SERVER_API_KEY", "dummy")
        ...
        try:
            ...
            logger.info("ScriptEngine: OpenAI互換API応答受信 (chars=%d)", len(response_text))
            return response_text
        except OSError as e:
            raise RuntimeError(
                f"OpenAI互換API接続エラー ({base_url}): {e}\n"
                "cocoro-llm-serverが起動しているか確認してください。"
            ) from e
        except Exception as e:
            raise RuntimeError(f"OpenAI互換API呼び出しエラー: {e}") from e
